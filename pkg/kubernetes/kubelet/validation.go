...
// 2. it checks if the container runtime socket path exists
...
func (s *service) validation() error {
	...
	if _, err := os.Stat(s.runtimeSocketPath); os.IsNotExist(err) {
		log.Error().Str("component", "kubelet").Msgf("container runtime socket path %s does not exist", s.runtimeSocketPath)
		return fmt.Errorf("container runtime socket path %s does not exist (check --container-runtime-endpoint)", s.runtimeSocketPath)
	}
