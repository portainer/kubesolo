...
For cases that require authentication, custom CA certificates, TLS verification skipping, or multiple fallback mirrors, manage the `hosts.toml` file directly and omit the `--registry-mirror` flag for that registry. KubeSolo will never overwrite a `hosts.toml` for a registry it has not been explicitly configured to manage.
