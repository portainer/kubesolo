...
// InstallerName returns the release asset name of the kubesoloctl binary for
// this host, e.g. "kubesoloctl-linux-amd64".
// Unlike ArchiveName, the kubesoloctl binary is published as a single asset per
// arch with no libc split — the binary is a pure-Go CGO_ENABLED=0 binary
// that runs on both glibc and musl systems.
func (s *SystemInfo) InstallerName() string {
	return fmt.Sprintf("kubesoloctl-%s-%s", s.OS, s.ArchiveSuffix)
}
